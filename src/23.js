...
const vertexShader = glslify(/* glsl */`
  ...
  void main() {
    ...
    newPosition.x *= rotation.x;
    newPosition.y *= rotation.y;
...
const sketch = ({ context, width, height }) => {
  ...
  let material = new THREE.ShaderMaterial({
    ...
    fragmentShader,
    uniforms: {
      rotation:{value: new THREE.Vector4(0., 0., 0., 0.)}
    },
    wireframe: true
  })
  ...
  for (var x = 0; x < 100; x++) {
    ...
    cube.rotation.x = .05 * x
    cube.rotation.y = .05 * x
    cube.rotation.z = .05 * x
    group.add(cube);
  }
  ...
  return {
    ...
    render({ time, deltaTime }) {
      material.uniforms.rotation.value.set(Math.sin(time) * .1, Math.cos(time) * .1)
