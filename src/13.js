...
const fragmentShader = glslify(/* glsl */`
  ...
  void main() {
    ...
    vec2 circlePos = st;
    float c = circle(circlePos, 0.5, 0.3) * 5.5;
    ...
    vec4 colorB = vec4(u_color, 1.0);

    float finalMask = smoothstep(0.4, 0.45, n + c);
...
const sketch = ({ context, width, height }) => {
  ...
  let material = new THREE.ShaderMaterial({
    ...
    vertexShader,
    uniforms: {
      u_color: { type: 'vec3', value: new THREE.Color(0x00D8AE)},
